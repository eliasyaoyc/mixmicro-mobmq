...
/**
 * {@link ExceptionUtil} A collection of utility functions for dealing with exceptions and exception
 * workflow.
 *
...
public class ExceptionUtil {
  ...
  /**
   * Makes a string representation of the exception's stack trace, or "(null)", if the exception is
   * null.
   *
  ...
  /**
   * Checks whether the given exception indicates a situation that may leave the JVM in a corrupted
   * state, meaning a state where continued normal operation can only be guaranteed via clean
   * process restart.
   *
   * <p>Currently considered fatal exceptions are Virtual Machine errors indicating that the JVM is
   * corrupted, like {@link InternalError}, {@link UnknownError}, and {@link java.util.zip.ZipError}
   * (a special case of InternalError). The {@link ThreadDeath} exception is also treated as a fatal
   * error, because when a thread is forcefully stopped, there is a high chance that parts of the
   * system are in an inconsistent state.
   *
  ...
  /**
   * Checks whether the given exception indicates a situation that may leave the JVM in a corrupted
   * state, or an out-of-memory error.
   *
   * <p>See {@link ExceptionUtil#isJvmFatalError(Throwable)} for a list of fatal JVM errors. This
   ...
   * @return the origianl {@link OutOfMemoryError} if it already has the new error message or a new
   *     {@link OutOfMemoryError} with the new error message
   */
   ...
   * {@link ExceptionUtil#isJvmFatalError(Throwable)} for a definition of fatal errors.
   *
  ...
  /**
   * Rethrows the given {@code Throwable}, if it represents an error that is fatal to the JVM or an
   * out-of-memory error. See {@link ExceptionUtil#isJvmFatalError(Throwable)} for a definition of
   * fatal errors.
   *
   * @param t The Throwable to check and rethrow.
   */
  ...
  /**
   * Adds a new exception as a {@link Throwable#addSuppressed(Throwable) suppressed exception} to a
   * prior exception, or returns the new exception, if no prior exception exists.
   *
   * <pre>{@code
   ...
   * Throws the given {@code Throwable} in scenarios where the signatures do not allow you to throw
   * an arbitrary Throwable. Errors and RuntimeExceptions are thrown directly, other exceptions are
   * packed into a parent RuntimeException.
   *
   * @param t The throwable to be thrown.
  ...
  /**
   * Unpacks an {@link CompletionException} and returns its cause. Otherwise the given Throwable is
   * returned.
   *
  ...
  /**
   * Unpacks an specified exception and returns its cause. Otherwise the given {@link Throwable} is
   * returned.
   *
