...
public class FutureUtil {
				...
				(t, throwable) -> {
					if (throwable != null) {
						if (throwable instanceof CancellationException) {
							...
							throwable = ExceptionUtil.stripExecutionException(throwable);
				...
				composedActionFuture.whenComplete(
					(Object innerIgnored, Throwable innerThrowable) -> {
						if (innerThrowable != null) {
							resultFuture.completeExceptionally(ExceptionUtil.firstOrSuppressed(innerThrowable, outerThrowable));
	 ...
	 * Unlike {@link FutureUtil#waitForAll(Collection)}, the resulting future won't be completed directly
	...
	private static final class CompletionConjunctFuture extends ConjunctFuture<Void> {
		...
		private void completeFuture(Object ignored, Throwable throwable) {
			synchronized (lock) {
				...
				if (throwable != null) {
					globalThrowable = ExceptionUtil.firstOrSuppressed(throwable, globalThrowable);
				}
	...
	/**
	 * Converts Neptune time into a {@link Duration}.
	 *
	...
	private enum Delayer {
		...
		static final ScheduledThreadPoolExecutor DELAYER = new ScheduledThreadPoolExecutor(
			1,
			new ExecutorThreadFactory("NeptuneCompletableFutureDelayScheduler"));
