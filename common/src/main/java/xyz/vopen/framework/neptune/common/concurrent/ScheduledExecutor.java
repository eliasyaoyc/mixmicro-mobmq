...
public interface ScheduledExecutor extends Executor {
  ...
  /**
   * Executes the given callable after the given delay. The result of the callable is returned as a
   ...
   * after {@code initialDelay + 2*period} and so on. The task is executed until either an execution
   * fails, or the returned {@link ScheduledFuture} is cancelled.
   *
   ...
   * @return a ScheduledFuture representing the periodic task. This future never completes unless an
   *     execution of the given task fails or if the future is cancelled
   */
  ScheduledFuture<?> scheduleAtFixedRate(
      Runnable command, long initialDelay, long period, TimeUnit unit);

  /**
   * Executed the given command repeatedly with the given delay between the end of an execution and
   * the start of the next execution. The task is executed repeatedly until either an exception
   * occurs or if the returned {@link ScheduledFuture} is cancelled.
   *
