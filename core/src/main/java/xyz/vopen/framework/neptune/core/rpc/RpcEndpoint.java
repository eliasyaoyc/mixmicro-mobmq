...
import xyz.vopen.framework.neptune.core.concurrent.ComponentMainThreadExecutor;
import xyz.vopen.framework.neptune.core.concurrent.ScheduledFutureAdapter;
...
/**
 * {@link RpcEndpoint} Base class for RPC endpoints. Distributed components which offer remote
 * procedure calls have to extend the RPC endpoint base class. An RPC endpoint is backed by an
 * {@link RpcService}.
 *
...
public abstract class RpcEndpoint implements RpcGateway, AutoCloseableAsync {
  ...
  protected RpcEndpoint(final RpcService rpcService, final String endpointId) {
    this.rpcService = Preconditions.checkNotNull(rpcService, "rpcService");
    this.endpointId = Preconditions.checkNotNull(endpointId, "endpointId");
  ...
  /**
   * Triggers start of the rpc endpoint. This tells the underlying rpc server that rpc endpoint is
   ...
   *
   * <p>This method is called when the RpcEndpoint is being started. The method is guaranteed to be
   * executed in the main thread context and can be used to start the rpc endpoint in the context of
   * the rpc endpoint's main thread.
   *
   ...
   *
   * @return Future which is completed once all post stop actions are completed. If an error occurs
   *     this future is completed exceptionally
   */
   ...
   * @return Self gateway of the specified type which can be used to issue asynchronous rpc.
   */
  ...
  /**
   * Return a future which is completed with true when the rpc endpoint has been terminated. In case
   * of a failure, this future is completed with the occurring exception.
   *
   ...
   * the result of the callable. If the callable is not completed within the given timeout, then the
   * future will be failed with a {@link TimeoutException}.
   *
   ...
   *
   * <p><b>IMPORTANT:</b> This check only happens when assertions are enabled, such as when running
   * tests.
   *
  ...
  protected static class MainThreadExecutor implements ComponentMainThreadExecutor {
    ...
    public MainThreadExecutor(RpcServer rpcServer, Runnable mainThreadCheck) {
      this.gateway = Preconditions.checkNotNull(rpcServer);
